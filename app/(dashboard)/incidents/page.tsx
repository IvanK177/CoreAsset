...
import { getCachedIncidentsWithRelations, getCachedComputers, getCachedActiveEmployees, getCachedRoomRequests } from "@/lib/supabase/cached";

export default async function IncidentsPage({ searchParams }: { searchParams: Promise<{ selectedId?: string }> }) {
  const [allIncidents, computers, activeEmployees, roomRequests] = await Promise.all([
