...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      ...
      // The browser will not redirect on error, so we can show feedback
      console.error("Google OAuth error:", error.message);
    }
    // On success, the browser is automatically redirected to Google,
    // then back to /auth/callback — no need to reset isGooglePending
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#0f172a] to-[#1e3a5f] px-4">
      ...
      <div className="w-[420px] rounded-2xl bg-white p-8 shadow-xl">
        ...
        <Button
          variant="outline"
          className="w-full h-11 rounded-lg border-gray-200 text-gray-700 hover:bg-gray-50 gap-3 font-medium mb-5"
          disabled={pending || isDemoPending || isGooglePending}
          onClick={handleGoogleSignIn}
        >
          {isGooglePending ? (
            <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Подключение…</span>
          ) : (
            <>
              {/* Google "G" icon */}
