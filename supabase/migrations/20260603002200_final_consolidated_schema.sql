-- ============================================================
-- CoreAsset — Consolidated Schema Migration (Idempotent)
-- ============================================================

-- 1. Create custom enums safely if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
    CREATE TYPE public.user_role AS ENUM ('admin', 'employee', 'it_specialist', 'facilities', 'developer');
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'computer_status') THEN
    CREATE TYPE public.computer_status AS ENUM ('active', 'repair', 'decommissioned', 'storage');
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'incident_type') THEN
    CREATE TYPE public.incident_type AS ENUM ('hardware', 'software', 'network', 'other');
  END IF;
...
CREATE TABLE IF NOT EXISTS public.computer_templates (
...
DROP TRIGGER IF EXISTS trg_device_licenses_seats ON public.device_licenses;
...
DROP POLICY IF EXISTS computer_templates_select ON public.computer_templates;
...
DROP POLICY IF EXISTS device_licenses_select ON public.device_licenses;
...
-- 8. Enable Realtime Publications safely
-- (Checking and adding tables to publication)
DO $$
BEGIN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'room_requests') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.room_requests;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'incident_messages') THEN
...
DROP POLICY IF EXISTS "Allow public read on buckets" ON storage.buckets;
...
DROP POLICY IF EXISTS "Allow authenticated updates on ticket-attachments" ON storage.objects;
